...
- the evaluator's feedback text is free-form, differing run to run
- the generator's rewrite is only as good as its ability to incorporate that feedback

Recording a single concrete run into a cassette freezes all of that. Every assertion below then runs deterministically in CI with no API key, and `assert_step_uses_result_from` verifies that the second generator round actually consumed the evaluator's feedback. This catches the classic "agent regenerated but ignored the judge" bug without any real LLM call.
