...
# Set a dummy API key if none is configured. langchain-openai requires an API key at client construction time even when the underlying calls are being intercepted for cassette replay, so we provide a placeholder to keep replay-mode tests self-contained.
